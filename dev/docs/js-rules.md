...
| **pages/**      | Логика, специфичная для конкретной страницы                     | `index.js`                                                                |
...
- Имена полей формы, ключи в `FormData`, ключи в JSON ответах API — **snake_case** или **kebab-case** в зависимости от контракта API (в проекте для форм часто: `current_url`, `request_id`).
- Проект одноязычный (ru), языковые ключи не требуются.
