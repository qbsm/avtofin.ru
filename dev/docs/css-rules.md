...
| **sections/**   | Стили секций страницы (шапка, подвал, intro, контентные блоки)                                     | `header.css`, `footer.css`, `intro.css`, `runline.css`, `actions.css`, `combomap.css`                                                             |
| **components/** | Переиспользуемые блоки (карточки, формы, аккордеон, слайдер)                                       | `card.css`, `form-callback.css`, `accordion.css`, `heading.css`, `custom-checkbox.css`                                                           |
| **pages/**      | Стили, специфичные для конкретной страницы                                                         | `index.css`                                                                                                                                      |

**Правило:** один блок (секция или компонент) — один файл. Имя файла совпадает с именем блока в kebab-case: блок `.form-callback` → `components/form-callback.css`. В текущем проекте компоненты лежат рядом с twig-файлом в `dev/src/components/{name}/{name}.css`.
...
- **Блок:** один класс, без тега (например, `.card`, `.form-callback`).
